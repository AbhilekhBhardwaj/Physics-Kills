    ...
    <header className="fixed top-0 left-0 right-0 z-50 px-6 py-4 max-sm:py-5 flex items-center justify-between">
      ...
      <Link to="/" className="flex items-center gap-3 group" onClick={() => setIsOpen(false)}>
        <img
          src={ASSETS.logo}
          alt="Physics Kills"
          className="h-10 w-10 max-sm:h-16 max-sm:w-16 object-cover rounded-full ring-2 ring-pk-red/30 transition-transform duration-300 group-hover:scale-110"
        />
      ...
      <button
        ...
        className="relative z-[60] w-10 h-10 max-sm:w-16 max-sm:h-16 flex items-center justify-center rounded-lg border border-white/10 bg-black/40 backdrop-blur-md transition-all duration-300 hover:border-pk-red/50 hover:shadow-[0_0_15px_rgba(230,57,70,0.3)]"
        aria-label="Toggle menu"
      >
        {isOpen ? (
          <X className="w-5 h-5 max-sm:w-8 max-sm:h-8 text-pk-red" />
        ) : (
          <Menu className="w-5 h-5 max-sm:w-8 max-sm:h-8 text-white" />
        )}
      </button>
